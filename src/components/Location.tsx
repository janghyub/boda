            ...
            <div className="py-[120px]">
                <div className="text-white bg-map h-[1000px] bg-left-bottom">
                    <div className="w-[1280px] m-auto ">
                        <div className="text-190">Welcome!</div>
                        <div className="text-24">BODA LAB</div>
                        <div className="mt-[140px]">
                            <div className="text-32">Head Office</div>
                            {LOCATION_INFO_ITEMS.map((item, index) => (
                                <LocationInfo key={index} title={item.title} content={item.content} />
                            ))}
                        </div>
                    </div>
                </div>
...
const LOCATION_INFO_ITEMS = [
    { title: 'Address', content: '서울특별시 금천구 가산디지털2로 169-16, 907호 (가산하우스디퍼스타)' },
